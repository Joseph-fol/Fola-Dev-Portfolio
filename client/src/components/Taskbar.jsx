  ...
  return (
    ...
    >
      <div className="flex items-center">
        <div className="flex items-center gap-2 rounded-full px-3 py-2 text-xs" style={{ background: 'var(--surface-muted)', color: 'var(--surface-text-secondary)' }}>
          <IconCloud size={16} className="text-[#1a6ef5]" />
          <span>{city}</span>
          <strong style={{ color: 'var(--surface-text-primary)' }}>{temperature}</strong>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-2">
        {APPS.map((app) => {
          ...
          return (
            ...
            >
              <Icon size={18} />
              <span className="pointer-events-none absolute bottom-14 left-1/2 -translate-x-1/2 whitespace-nowrap rounded px-2 py-1 text-xs text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100" style={{ background: 'var(--surface-text-primary)' }}>
                {app.label}
              </span>
      ...
      <div className="flex items-center justify-end gap-3" style={{ color: 'var(--surface-text-primary)' }}>
        <button
          type="button"
          className="flex items-center gap-2 rounded-lg px-2 py-1.5"
          style={{
            background: 'var(--surface-muted)',
            color: settings.wifiOn && !settings.airplaneMode ? 'var(--accent-color)' : 'var(--surface-text-secondary)',
          }}
          onClick={(event) => {
            event.stopPropagation();
            onToggleQuickSettings();
        ...
        <div className="h-7 w-px" style={{ background: 'var(--surface-border)' }} />
        <div className="min-w-20 text-right">
          <div className="text-sm font-bold leading-tight">{formattedTime}</div>
          <div className="text-[10px] leading-tight" style={{ color: 'var(--surface-text-secondary)' }}>{formattedDate}</div>
        </div>
      </div>
    </div>
  );
